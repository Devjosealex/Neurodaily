...
export function TaskForm({ onSuccess, initialData, onCancel }: { onSuccess: () => void; initialData?: any; onCancel?: () => void }) {
  const { getToken } = useAuth();
  
  const [title, setTitle] = useState(initialData?.title || '');
  const [description, setDescription] = useState(initialData?.description || '');
  const [category, setCategory] = useState(initialData?.category || 'work');
  const [cognitiveLoad, setCognitiveLoad] = useState<CognitiveLoad>(initialData?.cognitiveLoad || 'medium');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialData) {
        await api.updateTask(initialData.id, payload);
      } else {
      ...
      if (err.errors) {
        ...
        setError(err.message || (initialData ? 'Error al actualizar la tarea.' : 'Error al crear la tarea. Revisa tus límites Free.'));
      }
    } finally {
      ...
      <div className="flex gap-2 pt-2">
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 py-2.5 bg-secondary text-secondary-foreground font-medium rounded-xl hover:bg-secondary/80 transition-colors"
          >
            Cancelar
          </button>
        )}
        <button
          ...
          className="flex-1 py-2.5 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center justify-center"
        >
          {isSubmitting ? <LoadingSpinner size="sm" className="text-white" /> : initialData ? 'Guardar Cambios' : 'Crear Tarea'}
        </button>
      </div>
    </form>
  );
